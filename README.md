...
> ⚠️ Google 的 **Gemini CLI 已废弃（EOL）**，继任者为 **Antigravity CLI（`agy`）**。本项目已将"前端/UI 多模型协作"的底层从 Gemini CLI 包装器（`geminimcp`，工具 `mcp__gemini__gemini`）切换为 **Antigravity CLI 包装器 [agy-mcp](https://github.com/Boulea7/agy-mcp)**（注册名 `agy`，主工具 `mcp__agy__agy`）。仍在使用旧 Gemini MCP 的用户，请按以下步骤迁移。
...
前置：本机已安装并可用 **Antigravity CLI（`agy`）**。随后安装 agy-mcp（社区项目，已发布至 PyPI）：

```bash
# 若尚未安装 uv
curl -LsSf https://astral.sh/uv/install.sh | sh          # Windows(PowerShell): irm https://astral.sh/uv/install.ps1 | iex

# 安装 agy-mcp（提供 agymcp / agy-doctor / agy-install-skill 等入口）
uv tool install agy-mcp

# 注册到 Claude Code（server 名为 agy，对应工具 mcp__agy__agy）
claude mcp add agy -s user --transport stdio -- agymcp

# 验证（不消耗真实 agy 额度）
agy-doctor
claude mcp list      # 期望看到 agy: ✓ Connected
```
...
- **Agy MCP**（替代已废弃的 Gemini MCP）：`uv tool install agy-mcp` 后执行 `claude mcp add agy -s user --transport stdio -- agymcp`
